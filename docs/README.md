...
$$k_B$$, the interaction consists in these steps :
	...
	- Opinion comparison: B compares its opinion vector $$$\mathbf{v}_B$$ with $$\mathbf{v}_A$$, given the subset of companies on which the two Users' opinion are different, a company's index $c$ is extracted according to a uniform distribution. If $$\mathbf{v}_A$$ and $\mathbf{v}_B$ are equal the interaction stops. 
	- Influence	
	\begin{itemize}
